...
class AsvzEnroller:
    ...
    def wait_until(enrollment_start):
        ...
        if (enrollment_start - current_time).seconds > login_before_enrollment_seconds:
            ...
            sleep_until_time_str = (current_time + timedelta(seconds=sleep_time)).strftime("%d/%m/%Y %H:%M:%S")
            logging.info("Sleep until {}".format(sleep_until_time_str))
    ...
    def __get_enrollment_time(driver):
        ...
        logging.info("Enrollment starts at {} {}".format(
            enrollment_start.strftime("%H:%M:%S"),
            enrollment_start.strftime("%d/%m/%Y")
        ))
    ...
    def __get_lesson_time(driver):
        ...
        logging.info("Lesson starts at {} {}".format(
            lesson_start.strftime("%H:%M:%S"),
            lesson_start.strftime("%d/%m/%Y")
        ))
        return lesson_start
...
def find_event_time(event_weekday, event_time):
    # Get the current date and time
    today = datetime.today()

    # Calculate the next occurrence of the specified day of the week
    weekday_int = time.strptime(WEEKDAYS[event_weekday], "%A").tm_wday
    next_event_date = today + timedelta((weekday_int - today.weekday()) % 7)

    # Parse the event_time in military format
    event_hour, event_minute = map(int, event_time.split(":"))

    # Create the exact datetime of the next event
    next_event = next_event_date.replace(hour=event_hour, minute=event_minute)

    return next_event


# Function to create an event in Google Calendar
def set_calendar_event(date, summary, location, description, duration_hours=1):
    SCOPES = ['https://www.googleapis.com/auth/calendar']
    client_secrets_file = r"..\client_secret_194861639108-efatt15qgd0o6u2dn8hnke3abt37ldc3.apps.googleusercontent.com.json"
    token_file = r"..\token.pickle"  # Path to save the user's token
    ...
    if os.path.exists(token_file) and os.path.getsize(token_file) > 0:
        with open(token_file, 'rb') as token:
            try:
                ...
                # The token file is corrupted, handle the exception
                credentials = None
    ...
    event = {
        ...
        'end': {
            'dateTime': (date + timedelta(hours=duration_hours)).isoformat(),
            'timeZone': 'Europe/Zurich',
        },
    }
    event_added = service.events().insert(calendarId='primary', body=event).execute()
    print(f"Event created: {event_added.get('htmlLink')}")


def main():
    # Load the JSON file with multiple entries
    ...
    for data in entries:
        ...
        if data['type'] == "lesson":
            lesson_url = f"{LESSON_BASE_URL}/tn/lessons/{data['lesson_id']}"
            enroller = AsvzEnroller(chromedriver_path, lesson_url, creds, data.get('proxy'))
        elif data['type'] == "event":
            lesson_url = f"{LESSON_BASE_URL}/tn/events/{data['event_id']}"
            enroller = AsvzEnroller(chromedriver_path, lesson_url, creds, data.get('proxy'))
        elif data['type'] == "training":
            start_time_obj = parse_and_validate_start_time(data['start_time'])
            ...
            logging.error(f"Unknown enrollment type: '{data['type']}'")
            continue

        event_time = find_event_time(data['weekday'], data['start_time'])
        logging.info("\n\tLesson time: {}".format(event_time.strftime("%H:%M:%S")))

        # Enroll in the lesson/event
        enroller.enroll()
        # Add event to Google Calendar
        set_calendar_event(event_time, "ASVZ - "+SPORTS_CODE[data['sport_id']], data['facility'], "Trainer: "+data.get('trainer'))
